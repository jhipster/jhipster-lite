...
describe('JwtAuthRepository', () => {
  describe('login', () => {
    it('should call the login endpoint with correct credentials and store the token', async () => {
      ...
      mockAxiosHttp.post.mockResolvedValue({ data: mockResponse });
      ...
      expect(mockAxiosHttp.post).toHaveBeenCalledWith('api/authenticate', expectedPayload);
  ...
  describe('currentUser', () => {
    it('should call the user endpoint and return the user data', async () => {
      ...
      expect(mockAxiosHttp.get).toHaveBeenCalledWith('api/account');
